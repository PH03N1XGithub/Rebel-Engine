...
public:
    ...
    {
        bool Enabled = true;
        float GridExtent = 0.0f; // <= 0 means infinite extent
        float CellSpacing = .10f;
        float MajorLineSpacing = 1.0f;
        float FadeDistance = 8.0f;
        float MinorLineWidth = 0.50f;
        float MajorLineWidth = 0.50f;
        Vector4 MinorColor = Vector4(0.482, 0.482, 0.482, 0.059);
        Vector4 MajorColor = Vector4(0.482, 0.482, 0.482, 0.118);
        Vector4 XAxisColor = Vector4(1.f, 0.f, 0.f, 0.118f);
        Vector4 YAxisColor = Vector4(0.f, 1.f, 0.f, 0.118f); 
    };

    RenderModule()
    {
        m_TickType = TickType::Render;
    }
    ...
    [[nodiscard]] GLuint GetPreviewTexture() const { return m_PreviewColorTexture; }
...
private:
    ...
    uint32 m_PreviewFBO = 0;
    uint32 m_PreviewColorTexture = 0;
    uint32 m_PreviewRBO = 0;
    ...
    uint32 m_Width = 1280;
    uint32 m_Height = 720;
    uint32 m_PreviewWidth = 0;
    uint32 m_PreviewHeight = 0;
    ...
    void InitPreviewTargets(uint32 width, uint32 height);
    ...
    void DrawEditorGrid(const Mat4& viewProj, const Vector3& cameraPos, uint32 viewportWidth, uint32 viewportHeight);
    void CollectSceneDraws(Scene& scene, OpenGLRenderAPI* ogl, AssetManager& assetManager);
    void DrawDebugLines(const Mat4& viewProj);

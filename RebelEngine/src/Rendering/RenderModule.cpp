...
#include "Engine/Rendering/OpenGLRenderAPI.h"
// --- stb_image ---
...
float AxisBandWeight(float axisDistance, float spacing)
{
    spacing = max(spacing, 1e-4);
    float normalized = abs(axisDistance) / spacing;

    if (normalized >= 4.0)
        return 0.0;

    float t = 1.0 - normalized / 4.0;
...
void main()
{
    ...
    float majorSpacing = max(u_MajorSpacing, 1e-4);
    vec2 gridOrigin = floor(u_CameraPos.xy / majorSpacing + 0.5) * majorSpacing;
    vec2 centeredPos = worldPos.xy - gridOrigin;

    float minor = GridLineFactor(centeredPos / max(u_CellSpacing, 1e-4), u_MinorLineWidth);
    float major = GridLineFactor(centeredPos / majorSpacing, u_MajorLineWidth);

    float axisX = 1.0 - clamp(abs(centeredPos.y) / (fwidth(centeredPos.y) * 2.25), 0.0, 1.0);
    float axisZ = 1.0 - clamp(abs(centeredPos.x) / (fwidth(centeredPos.x) * 2.25), 0.0, 1.0);

    float dist = length(worldPos.xy - u_CameraPos.xy);
    float fade = 1.0 - smoothstep(u_FadeDistance * 0.45, u_FadeDistance, dist);
    fade = pow(clamp(fade, 0.0, 1.0), 1.35);

    vec3 color = mix(u_MinorColor.rgb, u_MajorColor.rgb, major);
    float alpha = max(minor * u_MinorColor.a, major * u_MajorColor.a);

    float axisMinorSpacing = max(u_CellSpacing, 1e-4);
    float xAxisBand = AxisBandWeight(centeredPos.y, axisMinorSpacing);
    float zAxisBand = AxisBandWeight(centeredPos.x, axisMinorSpacing);
    float xAxisInfluence = clamp(max(axisX, minor * xAxisBand), 0.0, 1.0);
    float zAxisInfluence = clamp(max(axisZ, minor * zAxisBand), 0.0, 1.0);

    color = mix(color, u_XAxisColor.rgb, xAxisInfluence);
    alpha = max(alpha, xAxisInfluence * mix(u_MinorColor.a, u_XAxisColor.a, axisX));

    color = mix(color, u_ZAxisColor.rgb, zAxisInfluence);
    alpha = max(alpha, zAxisInfluence * mix(u_MinorColor.a, u_ZAxisColor.a, axisZ));
...
void RenderModule::InitPreviewTargets(uint32 width, uint32 height)
{
    if (m_PreviewFBO) { glDeleteFramebuffers(1, &m_PreviewFBO); m_PreviewFBO = 0; }
    if (m_PreviewColorTexture) { glDeleteTextures(1, &m_PreviewColorTexture); m_PreviewColorTexture = 0; }
    if (m_PreviewRBO) { glDeleteRenderbuffers(1, &m_PreviewRBO); m_PreviewRBO = 0; }
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        std::cerr << "Preview FBO not complete!\n";

    glBindFramebuffer(GL_FRAMEBUFFER, 0);
}

void RenderModule::CollectSceneDraws(Scene& scene, OpenGLRenderAPI* ogl, AssetManager& assetManager)
{
    ...
    for (auto e : skelView)
    {
        ...
        if (!skComp || !skComp->bIsVisible || !skComp->IsValid())
            continue;
        ...
        const uint32 objectId = skComp->GetECSHandle() != entt::null ? ((uint32)skComp->GetECSHandle() + 1u) : 0u;
        ...
        TArray<Mat4> previewSkinPalette;
        const TArray<Mat4>* skinPalette = &skComp->FinalPalette;
        if (skComp->FinalPalette.IsEmpty())
        {
            ...
            if (skeleton &&
                AnimationRuntime::BuildBindPoses(skeleton, localBindPose, globalBindPose) &&
                AnimationRuntime::BuildSkinPalette(skeleton, globalBindPose, previewSkinPalette))
            {
                skinPalette = &previewSkinPalette;

                if (skComp->bDrawSkeleton)
                    RenderSkeletalDebug(skeleton, model, globalBindPose);
            }
        }

        if (!skinPalette || skinPalette->IsEmpty())
            continue;

        const uint32 boneBase = static_cast<uint32>(m_FrameBoneData.size());
        for (int32 i = 0; i < skinPalette->Num(); ++i)
            m_FrameBoneData.push_back((*skinPalette)[i]);
    ...
    for (auto e : view)
    {
        ...
        if (!mc || !mc->bIsVisible || !mc->IsValid())
            continue;

        if ((uint64)mc->Mesh.GetHandle() == 0)
            continue;

        MeshAsset* mesh = dynamic_cast<MeshAsset*>(assetManager.Load(mc->Mesh.GetHandle()));
        if (!mesh)
            continue;
        ...
        const uint32 objectId = mc->GetECSHandle() != entt::null ? ((uint32)mc->GetECSHandle() + 1u) : 0u;
        ogl->SubmitDraw(mesh->Handle, mc->GetWorldTransform(), mc->Material.Id, objectId, 0);
    }
}

void RenderModule::DrawDebugLines(const Mat4& viewProj)
{
    ...
    drawList(xray, true);
    drawList(normal, false);
}

uint32 RenderModule::ReadPickID(uint32 viewportX, uint32 viewportY) const
{
    if (!m_PickFBO || viewportX >= m_Width || viewportY >= m_Height)
        return 0;
    ...
    const GLint y = (GLint)(/*(m_Height - 1u) - */viewportY);
...
void RenderModule::DrawEditorGrid(const Mat4& viewProj, const Vector3& cameraPos, const uint32 viewportWidth, const uint32 viewportHeight)
{
    ...
    glUniform2f(m_uGridViewportSizeLoc, (float)viewportWidth, (float)viewportHeight);
...
void RenderModule::Tick(float dt) {
    ...
    if (m_Width != m_ViewportWidth || m_Height != m_ViewportHeight) {
        ...
        m_RendererAPI->InitFBO(m_Width, m_Height, m_FBO, m_ColorTexture, m_RBO);
        InitPickingTargets(m_Width, m_Height);
    }
    ...
    CameraView camera = GEngine->GetActiveCamera(float(m_Width) / m_Height);
...

    {
        ...
        for (auto e : skelView)
        {
            ...
            const uint32 objectId =
                skComp->GetECSHandle() != entt::null ? ((uint32)skComp->GetECSHandle() + 1u) : 0u;
    ...
    }
    {
        ...
        if (GEngine->GetMode() == EngineMode::Editor)
            DrawEditorGrid(viewProj, camera.Position, m_Width, m_Height);
...
void RenderModule::RenderScenePreview(
    Scene& scene,
    const CameraView& camera,
    const uint32 width,
    const uint32 height,
    const bool bDrawGrid,
    const bool bDrawCollision)
{
    if (!m_RendererAPI || width == 0 || height == 0)
        return;
    ...
    {
        InitPreviewTargets(width, height);
    ...
    if (bDrawCollision)
    {
        if (PhysicsModule* physicsModule = GEngine->GetModuleManager().GetModule<PhysicsModule>())
        {
            if (PhysicsSystem* physics = physicsModule->GetPhysicsSystem())
    ...
    CollectSceneDraws(scene, ogl, assetModule->GetManager());
    ...
    if (bDrawGrid)
        DrawEditorGrid(viewProj, camera.Position, width, height);
